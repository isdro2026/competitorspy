...
async function generateCaption(schedule) {
    const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });

  let prompt;
    if (schedule.input_type === 'url') {
          const articleText = await fetchArticleText(schedule.input_value);
          prompt = 'Here is the text content of a web page:\n\n"""\n' + articleText + '\n"""\n\nWrite a professional LinkedIn post based on this content, focused on business value or industry angle, minimal hashtags. Vary the phrasing and angle from a typical post so repeated posts about this page do not read identically. Return ONLY a JSON object (no markdown, no explanation) with this exact shape:\n{ "caption": "the LinkedIn post text" }';
    } else {
...
export default async function handler(req, res) {
  ...
  try {
      ...
      for (const schedule of due || []) {
              try {
                        const caption = await generateCaption(schedule);
